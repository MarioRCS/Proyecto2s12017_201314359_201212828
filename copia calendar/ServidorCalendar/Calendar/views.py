...
def registrar(request):
	if request.method == 'POST':
		...
		error = "Contrasenia no valida. Debe ser al menos de 4 caracteres"
...
def Logine(request,dato1,dato2):

	respuesta=""
	if ldU.verificarUsuario(str(dato1), str(dato2)) == True:
		respuesta="true"
...
def RegistrarCalendar(request,dato1,dato2):

	usuario=str(dato1)
	contrasenia=str(dato2)
	ldU.insertarFinal(usuario, contrasenia)
